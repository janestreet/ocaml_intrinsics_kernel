...
(** [count_leading_zeros n] returns the number of most-significant zero bits before the
    ...
    that is 32. *)
val count_leading_zeros : int32 @ local -> int32
...
val count_leading_zeros_nonzero_arg : int32 @ local -> int32
...
(** [count_trailing_zeros n] returns the number of least-significant zero bits before the
    ...
    [n], that is 32. *)
val count_trailing_zeros : int32 @ local -> int32
...
val count_trailing_zeros_nonzero_arg : int32 @ local -> int32

(** [count_set_bits n] returns the number of bits that are 1 in [n]. *)
val count_set_bits : int32 @ local -> int32

(** Shift operations below differ from the corresponding [Stdlib.Int32.shift_*] operations
    in two ways:
...
val shift_left : int32 @ local -> int32 @ local -> int32 @ local

(** [Int32.shift_right x y] shifts [x] to the right by [y] bits. This is an arithmetic
    shift: the sign bit of [x] is replicated and inserted in the vacated bits. *)
val shift_right : int32 @ local -> int32 @ local -> int32 @ local
...
val shift_right_logical : int32 @ local -> int32 @ local -> int32 @ local

(** Intrinsics for unboxed types. *)
module Unboxed : sig
  val count_leading_zeros : int32# -> int32#
  val count_leading_zeros_nonzero_arg : int32# -> int32#
  val count_trailing_zeros : int32# -> int32#
  val count_trailing_zeros_nonzero_arg : int32# -> int32#
