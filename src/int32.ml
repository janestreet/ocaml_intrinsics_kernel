...
let[@inline always] shift_left x y =
  ...
  | false -> exclave_ Emu.Int32.shift_left x y
;;

let[@inline always] shift_right x y =
  match Common.available with
  | true -> exclave_ Stubs.shift_right x y
  | false -> exclave_ Emu.Int32.shift_right x y
;;

let[@inline always] shift_right_logical x y =
  match Common.available with
  | true -> exclave_ Stubs.shift_right_logical x y
  | false -> exclave_ Emu.Int32.shift_right_logical x y
;;

module Unboxed = struct
  external box : (int32#[@unboxed]) -> (int32[@local_opt]) @@ portable = "%box_int32"
  external unbox : (int32[@local_opt]) -> (int32#[@unboxed]) @@ portable = "%unbox_int32"
