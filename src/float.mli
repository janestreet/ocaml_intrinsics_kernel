...
val iround_current : float @ local -> int64
...
val round_current : float @ local -> float

(** Rounds a [float] to an integer [float] using the mode specified in the function name. *)

val round_down : float @ local -> float
val round_up : float @ local -> float
val round_towards_zero : float @ local -> float
